package us.heptet.magewars.game.events;

import com.fasterxml.jackson.databind.ObjectMapper;
import org.jmock.Expectations;
import org.jmock.Mockery;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.testng.annotations.AfterMethod;
import org.testng.annotations.BeforeMethod;
import org.testng.annotations.Test;
import us.heptet.magewars.domain.game.Mage;
import us.heptet.magewars.domain.game.Player;
import us.heptet.magewars.domain.game.SpellBook;
import us.heptet.magewars.game.GameSituation;
import us.heptet.magewars.game.phase.InitiativePhase;
import us.heptet.magewars.test.GameTestHelper;

import static org.testng.Assert.*;

/**
 * Created by jade on 04/08/2016.
 */
public class GameEventTest {
    private static Logger logger = LoggerFactory.getLogger(GameEventTest.class);
    private ObjectMapper objectMapper = new ObjectMapper();
    private Mockery mockery;
    private GameSituation gameSituation;
    GameTestHelper gameTestHelper = new GameTestHelper() ;

    @BeforeMethod
    public void setUp() throws Exception {
        logger.info(":here");


        mockery = new Mockery();
        gameSituation = mockery.mock(GameSituation.class);
        mockery.checking(new Expectations() {{
            allowing(gameSituation).getNumPlayers();
            allowing(gameSituation).getGameControl();
            allowing(gameSituation).getActingPlayer();
        }});
        objectMapper = new ObjectMapper();
    }

    @Test
    public void testSerialize() throws Exception {

        GameEvent gameEvent = new PhaseEvent(PhaseEvent.NEW_PHASE, new InitiativePhase(gameSituation));
        String value = objectMapper.writeValueAsString(gameEvent);
        System.out.println(value);

    }

    @Test
    public void testSerializeDeserialize() throws Exception {

        Player player = gameTestHelper.player1();
        Mage card = player.getMagePlayerCard().getCard();

        assertNotNull(card);
        SpellBook spellBook = new SpellBook(player);
        spellBook.addCard(gameTestHelper.enchantmentPlayerCard(player).getCard());
        player.setSpellBook(spellBook);
        GameEvent gameEvent = new GameEvent(GameEvent.PLAYER_INFO);
        String value = objectMapper.writeValueAsString(gameEvent);
        logger.debug("value as string is {}", value);
        GameEvent event = objectMapper.readValue(value, GameEvent.class);
        logger.info("{}", event);
        logger.info("{}", event.getPlayer());

    }


    @AfterMethod
    public void tearDown() throws Exception {


    }

}